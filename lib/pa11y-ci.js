...
function pa11yCi(urls, options) {
	return new Promise(async resolve => {
		...
		function getFailingIssueCount(issues, reportConfig) {
			switch (reportConfig.level) {
				case 'error':
					return issues.filter(i => i.type === 'error').length;
				case 'warning':
					return issues.filter(i => i.type === 'error' || i.type === 'warning').length;
				default:
		...
		function processResults(results, reportConfig, url) {
			const issueCount = getFailingIssueCount(results.issues, reportConfig);
			const withinThreshold = reportConfig.threshold ?
					issueCount <= reportConfig.threshold :
					false;

			let message = ` ${chalk.cyan('>')} ${url} - `;
			if (issueCount && !withinThreshold) {
				message += chalk.red(`${issueCount} issues`);
				log.error(message);
				report.results[url] = results.issues;
				report.issues += issueCount;
			} else {
				message += chalk.green(`${issueCount} issues`);
				if (withinThreshold) {
					message += chalk.green(
